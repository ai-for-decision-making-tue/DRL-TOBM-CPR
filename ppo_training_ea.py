...
def save_best_fn(policy):
    ...
    torch.save(policy.state_dict(), f"models/ppo_ea_gnn.pt")
    # torch.save(policy.state_dict(), f"models/ppo_ea_invff.pt")
...
if __name__ == "__main__":
        ...
        if "obs" in kwargs:
            obs_with_tensors = [
                {"graph_nodes": torch.from_numpy(obs['obs']["graph"].nodes).float(),
                 ...
                 "agent_locations": torch.from_numpy(obs['obs']["locs"]).long(),
    ...
    log_path = os.path.join("logs/ppo_ea")
