...
def save_best_fn(policy):
    ...
    torch.save(policy.state_dict(), f"models/ppo_na_gnn.pt")
    # torch.save(policy.state_dict(), f"models/ppo_na_invff.pt")
...
if __name__ == "__main__":
    ...
    def preprocess_function(**kwargs):
        if "obs" in kwargs:
            obs_with_tensors = [
                {"graph_nodes": torch.from_numpy(obs['obs']["graph"].nodes).float().to(device=device),
                 "graph_edge_links": torch.from_numpy(obs['obs']["graph"].edge_links).int().to(device=device),
                 "mask": torch.from_numpy(obs['obs']["mask"]).to(torch.int8).to(device=device),
                 "agent_id": obs['agent_id']}
                for obs in kwargs["obs"]]
            kwargs["obs"] = obs_with_tensors
        if "obs_next" in kwargs:
            obs_with_tensors = [
                {"graph_nodes": torch.from_numpy(obs['obs']["graph"][0]).float().to(device=device),
                 "graph_edge_links": torch.from_numpy(obs['obs']["graph"][2]).int().to(device=device),
                 "mask": torch.from_numpy(obs['obs']["mask"]).to(torch.int8).to(device=device),
                 "agent_id": obs['agent_id']}
    ...
    log_path = os.path.join("logs/ppo_na")
